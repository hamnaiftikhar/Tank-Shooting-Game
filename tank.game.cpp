...
#define OptionsW 300
#define OptionsH  900   
#define tSize 80              //tank size
#define MaxBullets 30        // max bullets at a time 30
#define MaxEnemies 3        // maximum enemies at a time
#define EnemyStep 2
...
class game
{
	public:
		...
		int bIndex = 0;                 //bullet_index
		
		int enemies[MaxEnemies][3];
		int eIndex = 0;                //enemy_index
...
class tank : public update
{
	public :
			...
			void DrawEnemies()  //calculations done with the help of internet
			...
			void Forward(int x, int y, int isEnemy)
			{
			   ...
			   void Move(int dir, int x, int y)  // direction of tank func
			{
			    setcolor(WHITE);
				if( dir == 1 )
					Forward(x,y,3);
...
int main() {
  ...
  initwindow(1700, 1500, "TANK SHOOTING GAME");
  ...
  for(int i=0; i<MaxBullets; i++){
  	g.bullets[i][0] = g.bullets[i][1] = g.bullets[i][2] = 0;
  }
  
  for(int i=0; i<MaxEnemies; i++){
  	g.enemies[i][0] = g.enemies[i][1] = g.enemies[i][2] = 0;
  }
  ...
  while(1){
     ...
     outtextxy(ScreenW + 100, 250, g.txtScore);
    ...
    g.HitBullet();
     ...
     if( g.PlayerEnemyCollision(dir, x, y) == 1){
     g.GameOver();
     	break;
     }
